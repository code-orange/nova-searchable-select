<template>
	<DefaultField :field="field" :errors="errors">
		<template #field>
			<draggable v-if="selectedResources.length" class="mb-3 searchable-select__selected-resources"
				...
				<template #item="{ element }">
					<div
						...
						<div class="flex items-center px-4 py-1">
							<div v-if="element.image_url" class="mr-3">
								<img :src="element.image_url" class="w-8 h-8 rounded-full block"/>
							</div>
							<div class="mr-2">{{ element.display }}</div>
							<div @click="removeResource(element)" class="cursor-pointer">
			...
			>
				...
				<div slot="default" v-if="selectedResource" class="flex items-center">
					<div v-if="selectedResource.image_url" class="mr-3">
						<img :src="selectedResource.image_url" class="w-8 h-8 rounded-full block"/>
					</div>
					{{ selectedResource.display }}
				</div>

				<template v-slot:option="slotProps" class="flex items-center">
					<div v-if="slotProps.option.image_url" class="mr-3">
						<img :src="slotProps.option.image_url" class="w-8 h-8 rounded-full block"/>
					</div>
		...
		getAvailableResources(query, use_resource_ids, max) {
			...
			if (this.field.isMultiple) {
				...
				if (this.selectedResourcesIds.length > 0) {
					...
					resourceIds = typeof this.selectedResourceId === 'string' ? JSON.parse(this.selectedResourceId) : this.selectedResourceId;
				}
